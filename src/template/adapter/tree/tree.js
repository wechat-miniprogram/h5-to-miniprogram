...
class Tree {
  constructor(pageId, root, nodeIdMap, document) {
    this.pageId = pageId
    this.root = document._$createElement(root, this)
  ...
  walk(ast, parentNode) {
    ...
    for (const child of children) {
      ...
      if (child.type === 'element') {
        childNode = document._$createElement(child, this)
      } else if (child.type === 'text') {
        childNode = document._$createTextNode(child, this)
      }
  ...
  query(selector, node) {
    ...
    return this.querySelector.exec(selector, node || this.root, {
      idMap: this.idMap,
      tagMap: cache.tagMap,
