...
test('adjust css content: normal tagname', async () => {
    const res = await adjust('a, p {width:12px;} #id {height: 100%;}')
    expect(res).toBe('a, p {  width:12px;}#id {  height: 100%;}')
})

test('adjust css content: map tagname', async () => {
    const res = await adjust('a, p > video {width:12px;}')
    expect(res).toBe('a, p > h5-video {  width:12px;}')
})

test('adjust css content: html tagname', async () => {
    const res = await adjust('a, html, p {width:12px;}')
    expect(res).toBe('a, page, p {  width:12px;}')
})

test('adjust css content: * tagname', async () => {
    const res = await adjust('* {background:url(http://a.do);}')
...
test('adjust css content: rem', async () => {
    const res = await adjust('#a {width: 10rem;height: 5rem;}', {
        rem: 25,
    })
