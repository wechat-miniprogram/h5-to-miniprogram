...
var _betterScroll = __webpack_require__(1);
...
(function ($, win, doc) {
    var index = {
        ...
        _initConfig: function _initConfig() {
            var _self = this;
            var defaultListWidth = 320;
            var clientWidth = win.innerWidth || doc.documentElement.clientWidth;
        ...
        _initScroll: function _initScroll() {
            ...
            _self.scroll = new _betterScroll2.default(wrapper, options);
        },
        bind: function bind() {
            var _self = this;
        ...
        load: function load(fn) {
            ...
            var next = _self.data.page - 0 + 1 <= len ? true : false;
            ...
            try {
                ...
                for (var _iterator = _mock2.default[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    _loop();
                }
            } catch (err) {
        ...
        update: function update(fn) {
            ...
            var next = _self.data.page <= _update2.default.length ? true : false;
            var result = [];
            ...
            try {
                for (var _iterator2 = _update2.default[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    var _value = _step2.value;

                    if (_value.page <= _self.data.page - 1) {
                        result = result.concat(_value.data);
                    }
                }
            } catch (err) {
        ...
        setHtml: function setHtml(data) {
            var _self = this;
            var fragments = _self.setFragements(data);
            $('.list-wrapper').each(function (index, item) {
                $(item).append(fragments[index]);
            });
        },
        initHtml: function initHtml(data) {
        ...
        setFragements: function setFragements(data) {
            //generate some fragement
            var _self = this;
            var count = _self.config['list_number'];
            var fragments = _self.getFragments(count);
            data.forEach(function (item, index) {
                var i = index % count;
                var block = _self.templ(item);
                fragments[i].appendChild(block);
            });
            return fragments;
        },
        getFragments: function getFragments(num) {
            var arr = [];
            for (var i = 0; i < num; i++) {
        ...
        createOperations: function createOperations() {
            ...
            return { bShow: bShow, bHide: bHide, tShow: tShow, tHide: tHide, up: up, down: down, aShow: aShow, aHide: aHide };
        },
