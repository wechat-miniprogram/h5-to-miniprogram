...
import updateData from '../mock/update'
...
(function ($, win, doc){
    const index = {
        ...
        load: function(fn){
            ...
            const next = _self.data.page - 0 + 1 <= len ? true : false;
            for(let value of mockData){
                if(value.page == _self.data.page){
                    setTimeout(() => {
        ...
        update: function(fn){
            ...
            const next = _self.data.page <= updateData.length ? true : false;
            let result = [];
            for(let value of updateData){
                if(value.page <= _self.data.page - 1){
